...
TikzPreviewController::TikzPreviewController(MainWidget *mainWidget)
{
    ...
    File::setTempDir(
            m_tempDir->path()); // this must happen before any object of type File is constructed
#endif
}
...
{
    // File
    m_exportAction = new Action(Icon(QLatin1String("document-export")), tr("E&xport"),
                                m_parentWidget, QLatin1String("file_export_as"));
    ...
    Action *exportEpsAction =
            new Action(Icon(QLatin1String("image-x-eps")), tr("&Encapsulated PostScript (EPS)"),
                       exportMenu, QLatin1String("file_export_eps"));
    ...
    Action *exportPdfAction = new Action(Icon(QLatin1String("application-pdf")),
                                         tr("&Portable Document Format (PDF)"), exportMenu,
                                         QLatin1String("file_export_pdf"));
    ...
    mimeTypes << QLatin1String("png") << QLatin1String("jpeg") << QLatin1String("tiff")
              << QLatin1String("bmp");
    mimeTypeNames << tr("Portable Network &Graphics")
                  << tr("&Joint Photographic Experts Group Format")
                  << tr("&Tagged Image File Format") << tr("&Windows Bitmap");
    for (int i = 0; i < mimeTypes.size(); ++i) {
        Action *exportImageAction =
                new Action(Icon(QLatin1String("image-") + mimeTypes.at(i)),
                           mimeTypeNames.at(i) + QLatin1String(" (") + mimeTypes.at(i).toUpper()
                                   + QLatin1Char(')'),
                           exportMenu, QLatin1String("file_export_") + mimeTypes.at(i));
    ...
    // View
    m_procStopAction = new Action(Icon(QLatin1String("process-stop")), tr("&Stop Process"),
                                  m_parentWidget, QLatin1String("stop_process"));
    ...
    m_shellEscapeAction =
            new ToggleAction(Icon(QLatin1String("application-x-executable")), tr("S&hell Escape"),
                             m_parentWidget, QLatin1String("shell_escape"));
    m_shellEscapeAction->setStatusTip(tr("Enable the \\write18{shell-command} feature"));
    m_shellEscapeAction->setWhatsThis(tr(
            "<p>Enable LaTeX to run shell commands, this is needed when you want to plot functions "
...
{
    QString currentFile;
    const QString extension = mimeType == QLatin1String("image/x-eps")
            ? QLatin1String("eps")
            : (mimeType == QLatin1String("application/pdf") ? QLatin1String("pdf")
                                                            : mimeType.mid(6));
    if (!url.isEmpty()) {
        ...
        currentFile += currentFileInfo.completeBaseName()
                + (m_tikzPreview->numberOfPages() > 1
                                   && mimeType != QLatin1String("application/pdf")
                           ? QLatin1String("_") + QString::number(m_tikzPreview->currentPage() + 1)
                           : QString())
...
void TikzPreviewController::exportImage()
{
    ...
    if (mimeType == QLatin1String("application/pdf")) {
        ...
        if (!m_tikzPreviewGenerator->generateEpsFile(m_tikzPreview->currentPage())) {
            MessageBox::error(m_parentWidget, tr("Export failed."),
...
void TikzPreviewController::printImage(QPrinter *printer)
{
    ...
    for (int i = startPage; i <= endPage; ++i) {
        if (i != startPage)
            printer->newPage();
        const QImage image =
                m_tikzPreview->renderToImage(printer->physicalDpiX(), printer->physicalDpiY(), i);
        if (!image.isNull()) {
            const double scaleFactor = qMin(double(painter.window().width()) / image.width(),
                                            double(painter.window().height()) / image.height());
            painter.drawImage(
                    QRect(0, 0, image.width() * scaleFactor, image.height() * scaleFactor), image,
                    image.rect());
        }
    }
    painter.end();
...
void TikzPreviewController::printPreviewImage()
{
    ...
    printDialog->setOptions(printDialog->options() | QAbstractPrintDialog::PrintPageRange
...
{
    if (templateStatus
        == TikzPreviewGenerator::ReloadTemplate) // old aux files may contain commands available in
                                                 // the old template, but not anymore in the new
                                                 // template
        m_tempDir->cleanUp();

    // the directory in which the pgf file is located is added to TEXINPUTS (and the directory of
    // the old pgf file is removed) before running latex
    const QString currentFileName = m_mainWidget->url().path();
    if (!m_currentFileName.isEmpty() && currentFileName != m_currentFileName)
        m_tikzPreviewGenerator->removeFromLatexSearchPath(
    ...
    m_tikzPreviewGenerator->abortProcess(); // abort still running process before starting a new one
                                            // (without this, if a process hangs, all subsequently
                                            // started processes are executed one after the other
                                            // when the user aborts the hanging process)
...
void TikzPreviewController::applySettings()
{
    ...
    m_tikzPreviewGenerator->setLatexCommand(
            settings.value(QLatin1String("LatexCommand"), QLatin1String("pdflatex")).toString());
    m_tikzPreviewGenerator->setPdftopsCommand(
    ...
    m_templateWidget->setEditor(
            settings.value(QLatin1String("TemplateEditor"), QLatin1String("")).toString());

    settings.beginGroup(QLatin1String("Preview"));
    m_tikzPreview->setShowCoordinates(
            settings.value(QLatin1String("ShowCoordinates"), true).toBool());
    m_tikzPreview->setCoordinatePrecision(
            settings.value(QLatin1String("ShowCoordinatesPrecision"), -1).toInt());
    m_tikzPreview->setBackgroundColor(
            settings.value(QLatin1String("PreviewBackgroundColor"), QColor(0, 0, 0))
                    .value<QColor>());
    settings.endGroup();
}

      ...
      m_hasZoomed(false)
{
    ...
    setBackgroundColor(settings.value(QLatin1String("PreviewBackgroundColor"), QColor(Qt::white))
                               .value<QColor>());
    settings.endGroup();
...
{
    if (m_hasZoomed) {
        setSceneRect(m_tikzScene->itemsBoundingRect()); // make sure that the scroll area is not
                                                        // bigger than the actual image
        m_hasZoomed = false;
    }
    if (m_infoWidget
        && m_infoWidget->isVisible()) // make sure that if the error that Gnuplot is not found is
                                      // shown at startup, then it is shown in the center of the
                                      // view; this does not happen automatically because the error
                                      // is generated before the main window becomes visible
        centerInfoLabel();
...
{
    m_zoomToAction->setZoomFactor(
            m_zoomFactor + (m_zoomFactor > 0.99 ? (m_zoomFactor > 1.99 ? 0.5 : 0.2) : 0.1));
}

void TikzPreview::zoomOut()
{
    m_zoomToAction->setZoomFactor(
            m_zoomFactor - (m_zoomFactor > 1.01 ? (m_zoomFactor > 2.01 ? 0.5 : 0.2) : 0.1));
}
...
void TikzPreview::showPreview(const QImage &tikzImage, qreal zoomFactor)
{
    ...
    m_oldZoomFactor = zoomFactor; // m_oldZoomFactor must be set here and not in the zoom functions
                                  // in order to avoid skipping some steps when the user zooms fast
    m_hasZoomed = true;
...
void TikzPreview::showPdfPage()
{
    ...
    if (!m_processRunning)
        Q_EMIT generatePreview(m_tikzPdfDoc, m_zoomFactor,
                               m_currentPage); // render the current pdf page to a QImage in
                                               // TikzPreviewRenderer (in a different thread)
}
...
void TikzPreview::pixmapUpdated(Poppler::Document *tikzPdfDoc, const QList<qreal> &tikzCoordinates)
{
    ...
    if (m_currentPage >= numOfPages) // if the new tikz code has fewer tikzpictures than the
                                     // previous one (this may happen if a new PGF file is opened in
                                     // the same window), then we must reset m_currentPage
    {
        m_currentPage = 0;
        m_previousPageAction->setEnabled(false);
        m_nextPageAction->setEnabled(true);
...
void TikzPreview::setInfoLabelText(const QString &message,
...
void TikzPreview::setProcessRunning(bool isRunning)
{
    ...
    if (isRunning)
        setInfoLabelText(tr("Generating image", "tikz preview status"),
...
void TikzPreview::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (m_showCoordinates && m_tikzCoordinates.length() >= offset + 6) {
        ...
        const qreal unitY =
                m_tikzCoordinates.at(1 + offset); // unit length in y-direction in points
        const qreal minX =
                m_tikzCoordinates.at(2 + offset); // minimum x-coordinate on the figure in points
        const qreal maxX =
                m_tikzCoordinates.at(3 + offset); // maximum x-coordinate on the figure in points
        const qreal minY =
                m_tikzCoordinates.at(4 + offset); // minimum y-coordinate on the figure in points
        const qreal maxY =
                m_tikzCoordinates.at(5 + offset); // maximum y-coordinate on the figure in points

        if (unitX > 0 && unitY > 0) // this is not the case for 3D plots
        {
            int precisionX = m_precision; // the number of decimals used to display the x-coordinate
                                          // of the mouse pointer
            int precisionY = m_precision; // idem for the y-coordinate
            if (m_precision < 0) // in app/configgeneralwidget.cpp the precision is set to -1 if the
                                 // user chooses "Best precision", which we calculate now
            {
                qreal invUnitX = 1 / unitX;
                qreal invUnitY = 1 / unitY;
                for (precisionX = 0; invUnitX < 1;
                     ++precisionX) // make sure that some significant decimals are displayed (and
                                   // not numbers like 0.00)
                    invUnitX *= 10;

...
LineNumberWidget::LineNumberWidget(TikzEditor *editor) : QWidget(editor), m_editor(editor)
{
    setAutoFillBackground(true);

    QPalette p(palette());
    const QPalette standardPalette(
            p); // const QPalette standardPalette(QApplication::style()->standardPalette()); is
                // slow, since we are in the constructor palette() == QApplication::palette()
...
void LineNumberWidget::paintEvent(QPaintEvent *event)
{
    ...
    int top =
            (int)m_editor->blockBoundingGeometry(block).translated(m_editor->contentOffset()).top();
    ...
    while (block.isValid() && top <= event->rect().bottom()) {
        if (bottom >= event->rect().top()) {
            ...
            painter.drawText(0, top, width() - 4, lineHeight, Qt::AlignRight | Qt::AlignTop,

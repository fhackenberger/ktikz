...
void TikzEditor::highlightCurrentLine()
{
    ...
    */
    ...
    // update the area covering the previously highlighted line (updating the whole viewport is too
    // slow :-( )
    m_previousHighlightedLine.moveTop(
            m_previousHighlightedLine.top()
            - (verticalScrollBar()->value() - m_oldVerticalScrollBarValue + 0.5)
                    * m_previousHighlightedLine.height());
    m_previousHighlightedLine.setHeight(
            2 * m_previousHighlightedLine.height()); // should be large enough to cover the previous
                                                     // line (ugly hack :-( )
...
void TikzEditor::matchBrackets()
{
    ...
    if (!isBracket(document->characterAt(pos))
        && (pos <= 0
            || !isBracket(document->characterAt(--pos)))) // we do --pos because we want to match
                                                          // the bracket at both sides of the cursor
        return;
    ...
    int inc = 1;
    if (car == QLatin1Char('('))
        matchCar = QLatin1Char(')');
    else if (car == QLatin1Char('{'))
        matchCar = QLatin1Char('}');
    else if (car == QLatin1Char('['))
        matchCar = QLatin1Char(']');
    else {
    ...
    for (; pos >= 0 && pos < document->characterCount(); pos += inc) {
        if (document->characterAt(pos)
            == car) // if the brackets are nested, then don't match the closing bracket of the
                    // nested open bracket, e.g. in (()), don't match the first ) with the first (
...
void TikzEditor::printWhiteSpaces(QPainter &painter)
{
    ...
    for (QTextBlock block = firstVisibleBlock(); block.isValid(); block = block.next()) {
        if (blockBoundingGeometry(block).top()
            > viewport()->height()) // only paint white spaces in the visible part of the text
            break;
        ...
        for (int i = 0; i < textLength; ++i) {
            ...
            if (m_showWhiteSpaces && text.at(i) == QLatin1Char(' ')) {
                ...
                paintSpace(painter, rect.x() + spaceWidth / 2.0, rect.y() + fontHeight / 2.0,
                           spaceWidth);
            } else if (m_showTabulators && text.at(i) == QLatin1Char('\t')) {
                if (painter.pen() != tabulatorsPen)
                    painter.setPen(tabulatorsPen);
                paintTabstop(painter, rect.x() + spaceWidth / 2.0, rect.y() + fontHeight / 2.0,
                             spaceWidth);
            }
        }
    }
...
{
    QTextCursor cursor = textCursor();
    Q_EMIT cursorPositionChanged(cursor.blockNumber() + 1,
...
QString TikzEditor::textUnderCursor() const
{
    ...
    int position;
    for (position = cursor.position() - 1; position > 0;
         --position) // find the beginning of the word
    {
        const QChar character = document->characterAt(position);
        if (character.isSpace() // if the current char is a whitespace, then we have reached the
                                // beginning of the word
            || character == QLatin1Char('[')
            || character == QLatin1Char(',')) // these characters also delimit the beginning of the
                                              // word (the beginning of a TikZ option)
        {
            ++position;
            break;
        } else if (character == QLatin1Char('\\')
                   || position == startOfLine) // these characters also delimit the beginning of the
                                               // word (the beginning of a TikZ command)
        {
            break;
        }
    }
...
void TikzEditor::keyPressEvent(QKeyEvent *event)
{
    ...
    // the first time End is pressed moves the cursor to the end of the line, the second time to the
    // end of the block
    else if (event->key() == Qt::Key_Home && !(modifier & Qt::ControlModifier)
        ...
        setTextCursor(cursor);
        ensureCursorVisible();
    } else if (event->key() == Qt::Key_End && !(modifier & Qt::ControlModifier)
               && !(modifier & Qt::ShiftModifier)) {
        QTextCursor cursor = textCursor();
        const int oldPosition = cursor.position();
    ...
    // keys that change the content without moving the cursor may alter the brackets too
             ...
             || (event->key() == Qt::Key_Z && (modifier & Qt::ControlModifier))) {
    ...
    // go to next argument in text inserted with code completion
        ...
        // the following is done when there is no multiline selection and no s_completionPlaceHolder
        // can be found
        cursor.movePosition(QTextCursor::StartOfBlock, QTextCursor::KeepAnchor);
        QString selectedText = cursor.selectedText();
        if (selectedText.remove(QLatin1Char(' ')).remove(QLatin1Char('\t')).isEmpty()
...
void TikzEditor::toggleUserBookmark(int lineNumber)
{
    ...
    m_userBookmarks.append(lineNumber); // if lineNumber is larger than any number already in the
                                        // list, then we insert lineNumber at the end of the list
    m_lineNumberArea->update();
}
...
void TikzEditor::recalculateBookmarks(int position)
{
    ...
    if (addedLines != 0) {
        for (int i = 0; i < m_userBookmarks.length(); ++i) {
            // if a line containing a bookmark is removed, then the bookmark itself must also be
            // removed
            if (addedLines < 0 && lineNumber <= m_userBookmarks.at(i)
                && lineNumber - addedLines > m_userBookmarks.at(i)) {
                m_userBookmarks.removeAt(i);
                --i;
            } else if (lineNumber <= m_userBookmarks.at(i)) // shift all bookmarks that come after
                                                            // the insertion or deletion of lines

      ...
      m_replaceCurrentWidget(0)
{
    ...
    /*
            ...
            tikzHighlighter->rehighlight(); // avoid that textEdit emits the signal
       contentsChanged() when it is still empty tikzController = new TikzPngPreviewer(textEdit);
    */
...
void TikzEditorView::setFont(const QFont &editorFont)
{
    ...
    // Setting a stylesheet is faster than using QPlainTextEdit::setFont() (according to valgrind),
    // so here we go:
    QString fontStyle;
    QString fontWeight;
    ...
    case QFont::StyleOblique:
        fontStyle = QLatin1String("oblique;");
        break;
    default:
    ...
    case QFont::Light:
        fontWeight = QLatin1String("250;");
        break;
    case QFont::Normal:
        fontWeight = QLatin1String("normal;");
        break; // 400 according to http://www.w3.org/TR/CSS2/fonts.html#propdef-font-weight
    case QFont::DemiBold:
        fontWeight = QLatin1String("550;");
        break;
    case QFont::Bold:
        fontWeight = QLatin1String("bold;");
        break; // 700
    case QFont::Black:
        fontWeight = QLatin1String("850;");
        break;
    default:
        fontWeight = QLatin1String("normal;");
        break;
    ...
    m_tikzEditor->setStyleSheet(
            QString(QLatin1String("QPlainTextEdit { font-family: %1; font-size: %2; font-style: "
                                  "%3; font-weight: %4; text-decoration: %5; }"))
    ...
    // XXX dirty hack: make sure the correct font size is set, the stylesheet seems to ignore font
    // sizes :(
...
void TikzEditorView::createActions()
{
    ...
    m_copyAction->setWhatsThis(
            tr("<p>Copy the current selection's contents to the clipboard.</p>"));
    m_pasteAction->setWhatsThis(
            tr("<p>Paste the clipboard's contents into the current selection.</p>"));
    m_selectAllAction->setWhatsThis(tr("<p>Select all the content.</p>"));

    Action *action;
    action = new Action(Icon(QLatin1String("format-indent-more")), tr("&Indent..."), this,
                        QLatin1String("edit_indent"));
    ...
    m_editActions.at(8)->setWhatsThis(
            tr("<p>Search and replace a piece of text in the document.</p>"));
...
void TikzEditorView::applySettings()
{
    ...
    // set editor font
    m_tikzEditor->setShowLineNumberArea(
            settings.value(QLatin1String("ShowLineNumberArea"),
                           ConfigEditorWidget::defaultSetting(QLatin1String("ShowLineNumberArea")))
                    .toBool());
    QFont editorFont;
    editorFont.fromString(settings.value(QLatin1String("Font"),
                                         ConfigEditorWidget::defaultSetting(QLatin1String("Font")))
                                  .toString());
    setFont(editorFont);

    // set colors
    m_tikzEditor->setShowWhiteSpaces(
            settings.value(QLatin1String("ShowWhiteSpaces"),
                           ConfigEditorWidget::defaultSetting(QLatin1String("ShowWhiteSpaces")))
                    .toBool());
    m_tikzEditor->setShowTabulators(
            settings.value(QLatin1String("ShowTabulators"),
                           ConfigEditorWidget::defaultSetting(QLatin1String("ShowTabulators")))
                    .toBool());
    m_tikzEditor->setShowMatchingBrackets(
            settings.value(QLatin1String("ShowMatchingBrackets"),
                           ConfigEditorWidget::defaultSetting(
                                   QLatin1String("ShowMatchingBrackets")))
                    .toBool());
    m_tikzEditor->setWhiteSpacesColor(
            settings.value(QLatin1String("ColorWhiteSpaces"),
                           ConfigEditorWidget::defaultSetting(QLatin1String("ColorWhiteSpaces")))
                    .value<QColor>());
    m_tikzEditor->setTabulatorsColor(
            settings.value(QLatin1String("ColorTabulators"),
                           ConfigEditorWidget::defaultSetting(QLatin1String("ColorTabulators")))
                    .value<QColor>());
    m_tikzEditor->setMatchingColor(settings.value(QLatin1String("ColorMatchingBrackets"),
                                                  ConfigEditorWidget::defaultSetting(
                                                          QLatin1String("ColorMatchingBrackets")))
                                           .value<QColor>());
    m_tikzEditor->setHighlightCurrentLine(
            settings.value(QLatin1String("ShowHighlightCurrentLine"),
                           ConfigEditorWidget::defaultSetting(
                                   QLatin1String("ShowHighlightCurrentLine")))
                    .toBool());
    m_tikzEditor->setHighlightCurrentLineColor(
            settings.value(QLatin1String("ColorHighlightCurrentLine"),
                           ConfigEditorWidget::defaultSetting(
                                   QLatin1String("ColorHighlightCurrentLine")))
                    .value<QColor>());
    settings.endGroup();
...
void TikzEditorView::showCursorPosition()
{
        ...
        emit cursorPositionChanged(cursor.blockNumber() + 1, cursor.position() -
cursor.block().position() + 1);
}
*/
        ...
        if (!isUnindenting) {
            ...
            while (textCursor.position() < end && go) {
                for (int i = 0; i < numOfInserts; ++i) {
                    ...
                    ++end; // when a character is inserted, textCursor.selectionEnd() is shifted by
                           // 1 character, in order to let the test in the while-loop behave
                           // correctly, we must increment end
                }
            ...
            while (textCursor.position() < end && go) {
                for (int i = 0; i < numOfInserts; ++i) {
                    if (textCursor
                                .atBlockEnd()) // when the line is empty go to the next line instead
                                               // of selecting the newline character (possibly
                                               // together with characters of the next line)
                        break;
                    textCursor.movePosition(QTextCursor::NextCharacter, QTextCursor::KeepAnchor);
                    if (textCursor.selectedText() == insertChar) {
                        textCursor.removeSelectedText();
                        --end; // when a character is removed, textCursor.selectionEnd() is shifted
                               // by 1 character, in order to let the test in the while-loop behave
                               // correctly, we must decrement end
                    }
                }
                go = textCursor.movePosition(QTextCursor::NextBlock, QTextCursor::MoveAnchor);
            }
...
bool TikzEditorView::search(const QString &text, QTextDocument::FindFlags flags, bool startAtCursor,
...
void TikzEditorView::replace(const QString &text, const QString &replacement,
                             QTextDocument::FindFlags flags, bool startAtCursor)
{
    m_searchText = text;

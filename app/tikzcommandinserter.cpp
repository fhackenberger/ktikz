...
static TikzCommand newCommand(const QString &name, const QString &description,
                              const QString &command, const QString &highlightString, int dx,
                              int dy, int type)
{
    // type:
    //   0: plain text
...
static QString translateOptions(const QString &text)
{
    ...
    for (int pos = 0, oldPos = 0; pos >= 0;) {
        ...
        if (pos >= 0) {
            oldPos = pos;
            pos = text.indexOf(QLatin1Char('>'), pos); // option is between < and >
            translatedText += QCoreApplication::translate(
                    "TikzCommandInserter",
                    text.mid(oldPos + 1, pos - oldPos - 1).toLatin1().data());
        }
    }
...
static TikzCommandList getChildCommands(QXmlStreamReader *xml, QList<TikzCommand> *tikzCommandsList)
{
    ...
    commandList.title = QApplication::translate(
            "TikzCommandInserter",
            xml->attributes().value(QLatin1String("title")).toString().toLatin1().data());
    ...
    while (xml->readNextStartElement()) {
        if (xml->name() == QLatin1String("item")) {
            ...
            QString name = QApplication::translate(
                    "TikzCommandInserter",
                    xmlAttributes.value(QLatin1String("name")).toString().toLatin1().data());
            ...
            if (description
                        .isEmpty()) // if both name and description are empty, setting the
                                    // description first ensures that name is also set to insertion
                description = insertion;
            if (name.isEmpty()) {
                name = description;
                description.remove(QLatin1Char('&')); // we assume that if name.isEmpty() then an
                                                      // accelerator is defined in description
            }
            if (type.isEmpty())
                type = QLatin1Char('0');

            TikzCommand tikzCommand = newCommand(
                    name, description, insertion, highlightString,
                    xmlAttributes.value(QLatin1String("dx")).toString().toInt(),
                    xmlAttributes.value(QLatin1String("dy")).toString().toInt(), type.toInt());
            tikzCommand.number = tikzCommandsList->size();
            tikzCommandsList->append(tikzCommand);
            commands << tikzCommand;
            xml->skipCurrentElement(); // allow to read the next start element on the same level:
                                       // this skips reading the current end element which would
                                       // cause xml.readNextStartElement() to evaluate to false
            ...
            commands << newCommand(QString(), QString(), QString(), QString(), 0, 0,
                                   -1); // the i-th command with type == -1 corresponds to the i-th
                                        // submenu (assumed in getMenu())
...
static TikzCommandList loadChildCommandsJson(QJsonObject sectionObject,
                                             QList<TikzCommand> *tikzCommandsList)
{
    TikzCommandList commandList;
    QList<TikzCommand> commands;

    if (sectionObject.contains(QLatin1String("title")))
        commandList.title = QCoreApplication::translate(
                "TikzCommandInserter",
                sectionObject.value(QLatin1String("title")).toString().toLatin1().data());
        ...
        if (commandsArrayObject.isArray()) {
            ...
            for (int i = 0; i < commandsArray.size(); ++i) {
                ...
                if (commandObject.contains(QLatin1String("commands"))) {
                    commands << newCommand(QString(), QString(), QString(), QString(), 0, 0,
                                           -1); // the i-th command with type == -1 corresponds to
                                                // the i-th submenu (assumed in getMenu())
                    commandList.children << loadChildCommandsJson(commandObject, tikzCommandsList);
                } else if (type == -1) {
                    commands << newCommand(QString(), QString(), QString(), QString(), 0, 0, 0);
                } else {
                    QString name =
                            QCoreApplication::translate("TikzCommandInserter",
                                                        commandObject.value(QLatin1String("name"))
                                                                .toString()
                                                                .toLatin1()
                                                                .data());
                    QString description =
                            commandObject.value(QLatin1String("description")).toString();
                    QString insertion = commandObject.value(QLatin1String("insert")).toString();
                    QString highlightString =
                            commandObject.value(QLatin1String("highlight")).toString();
                    ...
                    if (name.isEmpty()) {
                        name = description;
                        description.remove(
                                QLatin1Char('&')); // we assume that if name.isEmpty() then an
                                                   // accelerator is defined in description
                    }

                    TikzCommand tikzCommand =
                            newCommand(name, description, insertion, highlightString,
                                       commandObject.value(QLatin1String("dx")).toInt(),
                                       commandObject.value(QLatin1String("dy")).toInt(), type);
                    tikzCommand.number = tikzCommandsList->size();
                    tikzCommandsList->append(tikzCommand);
                    commands << tikzCommand;
                }
...
static TikzCommandList loadCommandsJson(const QString &fileName,
                                        QList<TikzCommand> *tikzCommandsList)
{
    TikzCommandList commandList;
...
static QString removeOptionsAndSpecialCharacters(const QString &text)
{
    ...
    for (int i = 0; i < text.length(); ++i) {
        ...
        if (cat == QChar::Letter_Lowercase || cat == QChar::Letter_Uppercase
...
void TikzCommandInserter::updateDescriptionToolTip()
{
    ...
    if (action) {
        ...
        QToolTip::showText(
                menu->mapToGlobal(QPoint(rect.x() + rect.width(), rect.y() - rect.height() / 2)),
                QLatin1String("<p>") + description + QLatin1String("</p>"), menu, rect, 5000);
    }
}
    ...
    for (int i = 0; i < numOfCommands; ++i) {
        ...
        if (name.isEmpty()) // add separator or submenu
        {
            if (commandList.commands.at(i).type == 0) {
                menu->addSeparator();
            } else // type == -1, so add submenu; this assumes that the i-th command with type == -1
...
void TikzCommandInserter::addListWidgetItems(QListWidget *listWidget,
                                             const QPalette &standardPalette,
                                             const TikzCommandList &commandList, bool addChildren)
{
    ...
    for (int i = 0; i < commandList.commands.size(); ++i) {
        if (commandList.commands.at(i).type
            == -1) // if we have an empty command corresponding to a submenu, then don't add the
                   // command, the submenus will be added later
            continue;
...
{
    Q_ASSERT_X(m_commandsCombo, "TikzCommandInserter::showItemsInDockWidget()",
               "TikzCommandInserter::getDockWidget(QWidget *parent) should be run before using "
               "this function");
    Q_ASSERT_X(m_commandsStack, "TikzCommandInserter::showItemsInDockWidget()",
               "TikzCommandInserter::getDockWidget(QWidget *parent) should be run before using "
               "this function");
    QListWidget *tikzListWidget = qobject_cast<QListWidget *>(m_commandsStack->widget(0));
    QPalette standardPalette =
            QApplication::style()
                    ->standardPalette(); // this is slow, so we call this only once here and pass
                                         // this as argument to addListWidgetItems instead of
                                         // calling this each time in addListWidgetItems
    addListWidgetItems(tikzListWidget, standardPalette, m_tikzSections,
                       false); // don't add children
...
QDockWidget *TikzCommandInserter::getDockWidget(QWidget *parent)
{
    ...
    tikzDock->setFeatures(QDockWidget::DockWidgetClosable | QDockWidget::DockWidgetMovable
...
QStringList TikzCommandInserter::getHighlightTypeNames()
{
    ...
    highlightTypeNames << QLatin1String("Commands") << QLatin1String("Draw to")
...
QVector<HighlightingRule> TikzCommandInserter::getHighlightingRules()
{
    ...
    for (int i = 0; i < m_tikzCommandsList.size(); ++i) {
        QString command = m_tikzCommandsList.at(i).command.isEmpty()
                ? m_tikzCommandsList.at(i).description
                : m_tikzCommandsList.at(i).command;
...
#ifdef KTIKZ_USE_KTEXTEDITOR
    if (m_mainKFEdit) {
        Q_ASSERT_X(m_mainKFEdit,
                   "TikzCommandInserter::insertTag(const QString &tag, int dx, int dy)",
                   "m_mainEdit should be set using TikzCommandInserter::setEditor() before using "
                   "this function");
...
#endif
        Q_ASSERT_X(m_mainEdit, "TikzCommandInserter::insertTag(const QString &tag, int dx, int dy)",
                   "m_mainEdit should be set using TikzCommandInserter::setEditor() before using "
                   "this function");
        ...
        // else we are only inserting a string with no placeholders and no positioning, so the
        // cursor must come at the end of the string (this is done automatically by Qt)
        m_mainEdit->setFocus();
#ifdef KTIKZ_USE_KTEXTEDITOR
    }
#endif
